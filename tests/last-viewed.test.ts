 ...
 * viewed, every change re-records its last-viewed heads (KEYS.lastViewedHeads),
 * so remote edits arriving mid-view count as seen. Docs with only peek
 * subscriptions (home summary, source inspector, HyperFormula bridge) flip the
...
import { KEYS } from '../src/shared/storage-keys';
...
function makeEngine() {
  ...
  return { engine, kv, emitted, unseenEvents };
}
...
describe('seen-state machine', () => {
  ...
  it('keeps last-viewed current on changes while a non-peek sub is live (stays seen)', async () => {
    ...
    expect(kv.get(KEYS.lastViewedHeads)).toEqual({ d1: ['h2'] });
  ...
  it('flips unseen on a change seen only through a peek sub, emitting once (transition-only)', async () => {
    ...
    expect(kv.get(KEYS.lastViewedHeads)).toBeUndefined(); // peek never records
  });
  ...
  it('pruneSeenState drops the record and re-emits without the doc', async () => {
    ...
    expect(kv.get(KEYS.lastViewedHeads)).toEqual({});
    const events = unseenEvents();
