 ...
 * this.lastViewedHeads and persisted to the synced DriveSettings doc), so remote
 * edits arriving mid-view count as seen. Docs with only peek subscriptions (home
 ...
 * itself (into the DriveSettings doc) is covered by the engine settings tests;
 * here there is no loaded settings doc, so persistLastViewed is a no-op.
 */
