...
- **Relay (default).** `src/relay/relay.ts` is a stateless WebSocket relay (also embedded in `serve.ts` / dev via `relay-plugin.ts`) that does peer discovery and routes opaque encrypted bytes by `targetId`. It identifies itself with the all-zero `RELAY_PEER_ID` (`src/shared/relay-identity.ts`) and is never a keyhive member.
